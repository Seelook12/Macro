...
- **레시피 계층 구조**:
  ...
  23. **2026-02-09 : 상수/변수 하이브리드 입력**: `Delay`, `RepeatCount` 등 수치형 속성 입력 시 `DynamicIntInput` 컨트롤을 사용함. [123] 아이콘은 상수 모드, [{x}] 아이콘은 변수 선택 모드를 의미함.
  24. **2026-02-09 : 시스템 내부 변수**: `__` (언더바 2개)로 시작하는 변수명은 시스템 내부 런타임용으로 간주함. 이 변수들은 메모리상에만 존재하며 `.vars.json` 파일에 저장되지 않음.
--- 
